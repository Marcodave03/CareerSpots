...
const ComposeMessage = () => {
    ...
    const fetchReceiverName = async () => {
        try {
            const response = await axios.get("http://localhost:5000/getstaffbystaffid/" + id);
    ...
    const sendMessage = async (e: FormEvent<HTMLFormElement>) => {
        e.preventDefault();
        try {
            const selectedStaff = await axios.get("http://localhost:5000/getstaffbystaffid/"+id); 
            await axios.post(`http://localhost:5000/createmessage`, {
                "user_id": userID, 
                "receiver_id": selectedStaff.data.User.user_id, 
